...
class _UserItemWidgetState extends State<UserItemWidget> {
  ...
  Widget userItem(BuildContext context, User item,
    ...
    SizedBox(
      ...
      child: GestureDetector(
        ...
        onSecondaryTapDown: (details) => { mouseClickLogic.onSecondaryTapDown(context, details, item) }, // (
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              SizedBox(
                width: 168,
                child: Text(item.email, style: utils.styleTextUtil.commonTextStyle(),
              )
              ),
              //
              const SizedBox(width: 12),
              SizedBox(
                width: 84,
                child: Text(item.name1, style: utils.styleTextUtil.commonTextStyle(),
              )
              ),
              // 
              const SizedBox(width: 12),
              SizedBox(
                width: 84,
                child: Text(item.name2 != null ? item.name2! : "", style: utils.styleTextUtil.commonTextStyle(),
              )
              ),
              //
              const SizedBox(width: 12),
              SizedBox(
                width: 84,
                child: Text(item.name3 != null ? item.name3! : "", style: utils.styleTextUtil.commonTextStyle(),
              )
              ),
              //
              ...
              SizedBox(
                width: 96,
                child: Text(item.isBlocked.toString(), style: utils.styleTextUtil.commonTextStyle(),
              )
              ),
              //
              const SizedBox(width: 12),
              SizedBox(
                width: 96,
                child: Text(item.isDeleted.toString(), style: utils.styleTextUtil.commonTextStyle()),
              ),
              // размер хранилища
              SizedBox(
                width: 84,
                child: Text(utils.formatDataUtil.fBytes(item.storageSize) == "" ? "0 Байт": utils.formatDataUtil.fBytes(item.storageSize),
